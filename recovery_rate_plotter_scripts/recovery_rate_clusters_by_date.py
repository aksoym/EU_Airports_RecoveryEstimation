...
index_list = rr_feature_dataframe.index.tolist()
date_idx_list = [date_index.split('_')[0] for date_index in index_list]

rr_feature_dataframe['date_idx'] = date_idx_list

capacity_series = rr_feature_dataframe['avg_capacity']
normalized_avg_capacity = (capacity_series - capacity_series.min()) / (capacity_series.max() - capacity_series.min())


ax2.scatter(date_idx_list, normalized_avg_capacity, s=5, color='aqua', label='Norm. Avg. Capacity')
