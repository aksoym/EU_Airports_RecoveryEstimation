...
def calculate_pdots_on_df(airport_df: pd.DataFrame, infection_matrix: pd.DataFrame) -> pd.DataFrame:
    df = deepcopy(airport_df)
    
    for apt in df.index.unique("apt"):
        index = (slice(None), slice(None), [apt])
        df.loc[index, "recovery_rate"] = df.loc[index, "recovery_rate"].fillna(method="backfill")
        df.loc[index, "recovery_rate"] = df.loc[index, "recovery_rate"].fillna(df.loc[index, "recovery_rate"].median())
    
    p_dot_ndarray_list = []
    for date in tqdm(df.index.unique("date")):
        for tw in df.index.unique("tw"):
            index = (date, tw, slice(None))
            recovery_rate_vector = df.loc[index, "recovery_rate"].values
    ...
    df["p_dot"] = np.concatenate(p_dot_ndarray_list, axis=0)
