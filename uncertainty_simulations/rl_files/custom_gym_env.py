...
from rl_files.utils import fetch_single_flow_matrix, fetch_hourly_flow_matrices

pd.options.mode.chained_assignment = None

flight_flows = pd.read_pickle("../recovery_rate/recovery_rate_training_dataframes/data/all_time_flight_flow_df_rev1.pickle")
arr_flows = pd.read_pickle("../recovery_rate/recovery_rate_training_dataframes/data/hourly_arrival_matrix_updated_rev1.pickle")
dep_flows = pd.read_pickle("../recovery_rate/recovery_rate_training_dataframes/data/hourly_departure_matrix_updated_rev1.pickle")
...
class AirTrafficFlow(gym.Env):
    ...
    def step(self, action):
        ...
        print((row, column))
        if operation == 0:
            if self.hourly_matrix_list[3][row, column] >= 1:
                print(self.hourly_matrix_list[3][row, column])
                self.hourly_matrix_list[3][row, column] -= 1
                self.hourly_matrix_list[4][row, column] += 1
            ...
            if self.hourly_matrix_list[3][row, column] >= 1:
                print(self.hourly_matrix_list[3][row, column])
                self.hourly_matrix_list[3][row, column] -= 1
                self.hourly_matrix_list[5][row, column] += 1
        ...
        return resulting_observation, reward_from_observation, done, {"stability_metric": stability_metric}
    ...
    def reset(self, hourly_mat_list=None, recovery_rates=None):
        ...
        self.actual_operation = 0
        self.n_max_actions = np.rint(np.sum(self.hourly_matrix_list[3]) * self.operable_traffic_ratio)
    ...
    def _combine_hourly_flows_and_get_stability_matrices(self, hourly_matrix_list, recovery_rates):
        ...
        combined_flow_matrices = [pd.DataFrame(data=(hourly_matrix_list[idx] + hourly_matrix_list[idx+3]), index=list(range(self.n_apt)), columns=list(range(self.n_apt)))
                                       for idx in range(3)]
